...
/**
 * Small hack to prevent node 0.10 crash on {@code undefined}
 * as param for {@code path.join}. Actualy, {@code path.join} requires only
 * strings as arguments, so we replace {@code undefined} for "."
 * TODO(4u@): Fix path.join by defining relativePath as {@code string}
 */
var pathJoin = function (var_args) {
...
var compile = function (srcPath, dstPath, options, callback) {
	...
	var q = async.queue(function (task, done) {
		...
		if (task.processPath) {
			currentPath = pathJoin(srcPath, task.relativePath);
				...
				if (err) {
					...
					for (var i=0, l=items.length; i < l; i++) {
						if (items[i][0] != '.') { // skipping dot-files
							q.push({
								getStat: true,
								relativePath: pathJoin(task.relativePath, items[i]),
				...
				if (err) {
					...
					if (stat.isDirectory()) {
						...
						if (dstPath != null) {
							...
							var dirPath = pathJoin(dstPath, task.relativePath);
						...
						if (dstPath != null) {
							if (task.relativePath == null) {
								...
								dstFilePath = fileCompiler.getDefaultDstPath(currentPath, pathJoin(dstPath, task.previousRelativePath), options.ext);
							}
						}
						q.push({
							compile: true,
