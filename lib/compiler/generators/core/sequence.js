...
Sequence.prototype.onMsgToken = function(token) {
  this.appendMsg(token);
};
...
Sequence.prototype.appendMsg = function(token) {
	this.openOrSeparateIfNeed();

	if (!this.options.msg.isCommandAllowed) {
		throw Error('Command @msg is not allowed.');
	}

	var buffer = this.block.buffer;
	buffer.push(this.options.msg.getMsgFuncStr, "('");
	buffer.push(token.nameExpr.trim().replace(/'/g, "\\'"));
	buffer.push("')");
